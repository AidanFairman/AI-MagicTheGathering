       ...
       /* public static Card Target(ArtifactEnchant card, GameState state)
        ...
        public static void Defend(GameState state)
        {
            ...
            foreach(Card c in state.EnemyField)
            {
                if (c is Creature)
                {
                    Creature blocker = findBlockerDuringDefend(c as Creature, state);
                    if (blocker != null)
                    {
        ...
        private static Creature findBlockerDuringDefend(Creature c, GameState state)
        {
            return null;
        }
            ...
            if (grasslandsInPlay)
            {
                if (forestPriority)
                {
                    foreach (Card c in state.PlayerLibrary)
                    {
                        if (c is KnightCards.Forest || c is KnightCards.Treetop_Village)
                        {
                            return String.Format("Sacrifice Grasslands and Play {0}", c.CName);
                        }
                    }
                    foreach (Card c in state.PlayerLibrary)
                    {
